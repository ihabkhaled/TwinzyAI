...
/**
 * HTTP only: posts the image + consent flag + active language + result count as
 * multipart form data and validates the JSON response against the shared contract.
 * No business decisions live here — the service owns validation and orchestration.
 */
export const analyzeImageRequest = async (
  ...
  postMultipart(
    httpClient,
    GAME_ANALYZE_PATH,
    buildAnalyzeFormData(file, languageCode, resultCount),
