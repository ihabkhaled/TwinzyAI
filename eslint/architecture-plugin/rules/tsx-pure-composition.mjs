...
const isComponentFile = (filename) => {
  ...
  return normalized.endsWith('.tsx') && SCOPED_FOLDERS.some((folder) => isInFolder(normalized, folder));
};
...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'TSX files must stay pure composition — no built-in hooks, no fetch.',
    },
    schema: [],
    messages: {
      noBuiltinHook:
        '"{{hook}}" is not allowed in TSX. Move state/effects into a hook under hooks/.',
      noFetch: 'Do not call fetch from TSX. Use the gateway through a service and hook.',
    },
  },
  create(context) {
