...
- **Errors:** every user-facing failure is a typed, currently-used `AppError` subclass (`ValidationError`, `NotFoundError`, `PayloadTooLargeError`, `TooManyRequestsError`, `IntegrationError`, plus feature-owned status variants) carrying `messageKey = errors.<feature>.<key>`. Do not pre-create reserved auth/conflict classes; add one with its first real caller. The global filter returns the sanitized `ApiErrorResponse`-compatible envelope; stacks, provider errors, and file contents never leak. ([rules/16](../rules/16-backend-architecture.md), [rules/22](../rules/22-observability-logging.md))
...
- **Bounded lists:** every list an endpoint or service produces is hard-capped (max 100 items; the game candidate pool caps at 25 and displayed results at the shared 1–10 request bound). No unbounded accumulation in the in-memory stores. ([rules/07](../rules/07-performance-scalability.md))
...
- **Parallel candidate recall (flag-gated, OFF by default):** when `AI_PARALLEL_PIPELINE_ENABLED=true`, the text-only candidate-recall step fans out into bounded lanes via `CandidateRecallService`, gated by the process-global `AiStepConcurrencyGate` over `core/concurrency`'s `Semaphore` plus a per-analysis call budget. Extraction stays the only image call and the SSE contract is unchanged. ([docs/ai/concurrency-policy.md](../docs/ai/concurrency-policy.md), [ADR-004](../architecture/adrs/adr-004-parallel-ai-pipeline.md))
...
- **`architecture/application-layer-boundaries`** — use-case/service one-way rule; no pass-through tiers; only `trait-extraction.service.ts` may call image-capable AI provider methods.
