...
5b. `KIMI.md`, `GEMINI.md`, `GLM.md`, `QWEN.md`, `DEEPSEEK.md`, `OPENAI.md`, `ANTHROPIC.md`, and `MISTRAL.md` are compact agent bootstraps for the corresponding AI coding tools. They carry the Simple Code Ladder, the never-weaken list, and canonical links only — never full rule bodies (see rules/30 §3 and skills/prepare-agent-mirrors.md).
...
The frontend operating system (`apps/web`) is `Component → Hook → Service → Gateway` on the `app → modules/<feature> → shared → packages/<vendor>` anatomy: TSX is pure composition, state/effects live in hooks, logic in module helpers/mappers, HTTP only through the gateway/`packages/axios` wrapper, every user-facing string through i18n. Two mechanically-enforced standing rules apply on both sides: **no inline interface/type/reusable-const in any layer file** — definitions live in `types/`/`enums/`/`constants/`/`model/` (backend `architecture/no-inline-domain-definitions` incl. module-level value consts; frontend `frontend-architecture/no-inline-declarations`), and **components stay small** — `*.component.tsx`/`*.container.tsx` capped at `max-lines` 130 / `max-lines-per-function` 60 / `jsx-max-depth` (`eslint/frontend/component-size.config.mjs`); split into sub-components/sub-containers before the cap hits. Full bodies: `rules/02`–`rules/04`, `rules/12`, `rules/13`, `rules/frontend/`.
...
**Simple Code Ladder (permanent policy, rules/28–30):** before writing any code — need it → reuse existing owner → platform-native → existing wrapper → small pure helper → direct readable code → new abstraction only with a real current reason. Be lazy about code volume, never about reading, validation, security, privacy, AI safety, upload safety, tests, docs, a11y, i18n, or architecture. Simplicity means minimum SAFE code.
...
## AI Knowledge Operating System And Delivery Lanes (Permanent Policy)

The repository runs an incrementally compiled **Knowledge Operating System** (rules/31–36; owner's manual: [`knowledge/README.md`](knowledge/README.md)) with three planes: the **canonical plane** (this file plus `rules/ skills/ context/ memory/ agents/ testing/ support/ runbooks/ structure/ product/ domain/ contracts/ operations/ incidents/ quality/ knowledge/ docs/`), the **compiled plane** (`.ai/` — bootstrap, hot memory, quick router, manifests, indexes, context packs, graphs, hash snapshots; generated by `npm run knowledge:build`, committed, and never hand-edited), and the **execution plane** (the fast-task protocol in [`skills/resolve-task-context.md`](skills/resolve-task-context.md)).
...
1. **The game is free by default; paid gating exists only inside the recorded owner-gated paywall program.** No subscriptions, no accounts, no monetization outside that program — ever. *Owner-approved revision (2026-07-10, recorded in `docs/features/paypal-donations-and-paid-results/`):* one voluntary, clearly-labeled outbound **PayPal.me donation link** is permitted (env-driven handle `NEXT_PUBLIC_PAYPAL_ME_USERNAME`, strictly validated, hidden when unset); a paypal.me link can never gate results because payment completion is unverifiable. *Owner-approved supersession (2026-07-12, recorded in `docs/features/paypal-donations-and-paid-results/22-go-no-go.md` — SANDBOX-GO):* an **env-gated PayPal Orders v2 paywall** is permitted — capture-at-consumption bound to the request id (`custom_id === requestId`), server-authoritative price (`PAYMENT_PRICE_VALUE`), zod-parsed gateway responses, refund-on-undelivered-result, and **no persistence** (PayPal is the ledger; that record explicitly dropped the earlier durable-order-store and webhook gates). With blank `PAYPAL_CLIENT_ID`/`PAYPAL_CLIENT_SECRET` (the default) every payment path no-ops and the game is fully free; this free default may never be inverted. **LIVE mode is NOT approved**: `PAYPAL_ENV=live` and paywall-on production config stay forbidden until the four LIVE conditions recorded in `22-go-no-go.md` (deployed HTTPS origin; Business account after sandbox sign-off; owner-approved en+ar copy revision; recorded live smoke test) are met and recorded. Any further paywall scope change re-enters that program's gates.
...
/knowledge/      (authored Knowledge-OS definitions: routing, lanes, packs, budgets, golden sets)
/.ai/            (GENERATED acceleration plane — npm run knowledge:build; never hand-edited)
/structure/  /product/  /domain/  /contracts/  (canonical area docs)
/operations/  /incidents/  /quality/           (ops model, incident process, quality standards)
/docs/ai/        (AI pipeline knowledge area)
/scripts/knowledge/ (the knowledge compiler)
