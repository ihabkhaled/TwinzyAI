...
1. **The game is free.** No paywalls, no subscriptions, no payment capture, no gating of any feature or result on money — ever. *Owner-approved revision (2026-07-10, recorded in `docs/features/paypal-donations-and-paid-results/`):* one voluntary, clearly-labeled outbound **PayPal.me donation link** is permitted (env-driven handle `NEXT_PUBLIC_PAYPAL_ME_USERNAME`, strictly validated, hidden when unset). The app itself never processes, verifies, or records money. Any paid-results feature stays forbidden until the recorded paywall program in that folder passes its own gates (real gateway API + webhooks, durable order store, deployed HTTPS origin, revised privacy/consent copy) — a paypal.me link can never gate results because payment completion is unverifiable.
