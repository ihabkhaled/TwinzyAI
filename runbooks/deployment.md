...
1. All quality gates green on the exact release revision — same commands as CI (`.github/workflows/gate-*.yml`): `npm run lint`, `npm run typecheck`, `npm run test:unit`, `npm run test:coverage`, `npm run test:e2e:ci`, `npm run build`, `npm run security:audit` + `npm run security:scan` (0 HIGH/CRITICAL), `npm run knowledge:build`, `npm run knowledge:validate`, and `npm run knowledge:benchmark`. Final formatting precedes the Knowledge rebuild; every `.ai/` change is committed, `git diff --exit-code -- .ai` is clean, and remote `Gate - Lint` plus `Gate - Knowledge` are green. A hook mutation restarts the two local gates; a red remote gate stops deployment and later slices.
