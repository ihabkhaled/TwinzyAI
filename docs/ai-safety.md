...
- Every response is Zod-validated with strict shapes: the 221-field advanced grouped visible
  trait taxonomy (built from `trait-category.constants.ts`), bounded arrays and strings, up to 5
  candidates and up to 5 final style/vibe results, and a `promptVersion` literal of
