...
- the existing `app/sitemap.ts` remains the `/sitemap.xml` owner and publishes every first-class
  route for every supported language (10 routes × 12 languages), with complete hreflang clusters;
...
- `/contact` submits a strict shared `{ email, subject, message }` contract to
  `POST /api/v1/contact`;
