...
- [ ] **AI output filtered.** Every model response is Zod-validated (shared 221-field taxonomy, candidate pool ≤25, requested results 1–10), safety-filtered in English/Arabic (`AiSafetyService` + shared forbidden wording), and rejected when safety flags are not all false; disclaimer/fallback copy is server-owned.
